...
pytesseract.pytesseract.tesseract_cmd = ''  # Update this path
...
origins = [
    ...
    "http://127.0.0.1:5173", # Optionally, for localhost with a different port
...
# FastAPI-specific function to process multiple PDF uploads
@app.post("/process_pdfs/")
async def process_uploaded_pdfs(files: List[UploadFile] = File(...), output_dir: str = "output"):
    os.makedirs(output_dir, exist_ok=True)  # Ensure 'output' directory exists
    report_paths = []

    for index, file in enumerate(files, start=1):
        # Save each uploaded PDF to a temporary location
        pdf_path = os.path.join(output_dir, f"temp_{file.filename}")
        ...
        # Save the extracted content as a .txt file with a unique name
        report_txt_path = os.path.join(output_dir, f"file{index}.txt")
        save_report_as_txt(report_content, report_txt_path)

        # Optional: Remove the temporary PDF file after processing
        os.remove(pdf_path)

        report_paths.append(report_txt_path)

    return {"file_paths": report_paths}


# Start the FastAPI server (for local testing, use `uvicorn` to run the app)
# Command to run: `uvicorn app:app --reload` (where app is the filename)
